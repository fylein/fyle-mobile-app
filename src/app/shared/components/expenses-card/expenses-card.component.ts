...
export class ExpensesCardComponent implements OnInit {
  ...
  handleScanStatus(): void {
    ...
    if (!that.isOutboxExpense) {
      that.platformEmployeeSettingsService.get().subscribe((platformEmployeeSettings) => {
        if (
          platformEmployeeSettings.insta_fyle_settings.allowed &&
          platformEmployeeSettings.insta_fyle_settings.enabled &&
