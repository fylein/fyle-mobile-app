...
fdescribe('Ellipses Pipe', () => {
  ...
  it('should add ellipses to long text', () => {
    expect(pipe.transform(sampleLongText, ellipsisTestLength)).toBe(
      `${sampleLongText.slice(0, ellipsisTestLength)}...`
    );
  });

  it('should add ellipses to short text', () => {
    expect(pipe.transform(sampleShortText, ellipsisTestLength)).toBe(`${sampleShortText}`);
  });
