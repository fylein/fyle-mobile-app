...
describe('RealTimeFeedService', () => {
  ...
  describe('getCardType()', () => {
    ...
    it('should return visa if the input card number starts with 4', () => {
      expect(realTimeFeedService.getCardType('4111111111111111')).toBe(CardNetworkType.VISA);
    });

    it('should return mastercard if the input card number starts with 5', () => {
      expect(realTimeFeedService.getCardType('5555555555555555')).toBe(CardNetworkType.MASTERCARD);
    });

    it('should return card type others if the input card number does not start with 4 or 5', () => {
      expect(realTimeFeedService.getCardType('6111111111111111')).toBe(CardNetworkType.OTHERS);
    });
  });
