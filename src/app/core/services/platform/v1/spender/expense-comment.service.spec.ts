...
describe('ExpenseCommentService', () => {
  ...
  let spenderServiceSpy: jasmine.SpyObj<SpenderService>;
  let statusServiceSpy: jasmine.SpyObj<StatusService>;

  beforeEach(() => {
    spenderServiceSpy = jasmine.createSpyObj('SpenderService', ['get', 'post']);
  ...
  describe('getExpenseCommentsById', () => {
    it('should call spenderService.get and return expense comments', (done) => {
      spenderServiceSpy.get.and.returnValue(of({ data: [expenseCommentData] }));

      service.getExpenseCommentsById('txjt6agn0gBY').subscribe((res) => {
        expect(res).toEqual([expenseCommentData]);
        expect(spenderServiceSpy.get).toHaveBeenCalledOnceWith('/expenses/comments', {
  ...
  describe('post', () => {
    it('should call spenderService.post with correct data', (done) => {
      ...
      service.post(payload).subscribe((res) => {
        expect(spenderServiceSpy.post).toHaveBeenCalledOnceWith('/expenses/comments/bulk', {
          data: payload,
        });
