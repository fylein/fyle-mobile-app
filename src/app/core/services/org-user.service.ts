...
export class OrgUserService {
  ...
  getEmployeesByParams(params: Partial<EmployeeParams>): Observable<Partial<Employee>[]> {
    return this.spenderPlatformV1ApiService
      .get<PlatformApiResponse<Partial<Employee>[]>>('/employees', { params })
  ...
  getEmployeesBySearch(params: Partial<EmployeeParams>): Observable<Partial<Employee>[]> {
    if (params.or) {
      params.and = `(or${params.or},or(is_enabled=eq.true&has_accepted_invite=eq.true,is_enabled=eq.true&has_accepted_invite=eq.false))`;
    } else {
      params.or = '(is_enabled=eq.true&has_accepted_invite=eq.true,is_enabled=eq.true&has_accepted_invite=eq.false)';
    }
