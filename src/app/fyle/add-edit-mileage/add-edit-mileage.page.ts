...
export class AddEditMileagePage implements OnInit {
  ...
  getNewExpense(employeeId: string): Observable<Partial<UnflattenedTransaction>> {
    const defaultVehicle$ = forkJoin({
      vehicleType: this.transactionService.getDefaultVehicleType(),
      employeeSettings: this.platformEmployeeSettingsService.get(employeeId),
      orgSettings: this.orgSettingsService.get(),
      ...
      map(
        ({
          ...
          const mileageRateLabel = employeeSettings?.mileage_rate_labels;
          if (mileageRateLabel?.length > 0 && !mileageRateLabel.some((label) => vehicleType === label)) {
            vehicleType = employeeSettings.mileage_rate_labels[0];
          }
    ...
    return this.etxn$.pipe(
      switchMap((etxn) => {
        if (etxn.tx.project_id) {
          ...
          return forkJoin({
            orgSettings: this.orgSettingsService.get(),
            employeeSettings: this.platformEmployeeSettingsService.get(this.authService.getEou().ou.id),
          }).pipe(
            map(({ orgSettings, employeeSettings }) => {
              if (orgSettings.projects.enabled) {
                return employeeSettings && employeeSettings.default_project_id;
              }
            })
          );
        }
  ...
  ionViewWillEnter(): void {
    ...
    const orgSettings$ = this.orgSettingsService.get();
    const employeeSettings$ = this.platformEmployeeSettingsService.get(this.authService.getEou().ou.id);
