...
export class MyViewReportPage {
  ...
  getVendorName(etxn: Expense): string {
    ...
    if (category === 'mileage') {
      vendorName = (etxn.tx_distance?.toString || 0)?.toString();
  ...
  editReportName(): void {
    this.erpt$
      ...
      .subscribe((editReportNamePopoverDetails: { data: { reportName: string } }) => {
  ...
  async deleteReportPopup(erpt: ExtendedReport): Promise<void> {
    ...
    const { data } = await (deleteReportPopover?.onDidDismiss() as Promise<{ data: { status: string } }>);
  ...
  async shareReport(): Promise<void> {
    ...
    const { data } = await (shareReportModal?.onWillDismiss() as Promise<{ data: { email: string } }>);
  ...
  addComment(): void {
    if (this.newComment) {
      ...
      (this.commentInput as ElementRef<HTMLInputElement>).nativeElement.focus();
  ...
  showAddExpensesToReportModal(): void {
    ...
    from(addExpensesToReportModal)
      ...
      .subscribe((addExpensesToReportModalDetails: { data: { selectedTxnIds: string[] } }) => {
